...
class CRUD_Tasks_Test(TestCase):
    ...
    def setUp(self):
        ...
        task.objects.create(name='task',
                            description='something', creator=self.user,
                            executor=self.user, status=self.status)
        self.task = task.objects.get(id=1)

    def test_read_task(self):
        response = Client().get(reverse_lazy('task_view',
                                             kwargs={'pk': self.task.pk}),
                                follow=True)
        self.assertEqual(response.status_code, 200)

    def test_create_task(self):
        response = Client().post(reverse_lazy('task_create'),
                                 {'name': 'something'}, follow=True)
